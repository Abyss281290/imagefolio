...
class JCropZoom extends CWidget
{
     ...
     */
    public $callbackUrl = '';
	public $triggerCustomData = array();
...
DTE;
    ...
    {
		$customData = CJavaScript::encode($this->triggerCustomData);
        $onServerCropped = CJavaScript::encode($this->onServerHandled);
		
		if($this->cropTriggerFunction) {
			$jsListenCrop = <<<CROP_HANDLE
				$(function(){
					$("#{$this->cropTriggerId}").click({$this->cropTriggerFunction});
				});
CROP_HANDLE;
		} else {
			$jsListenCrop = <<<CROP_HANDLE
				$(function(){
					$("#{$this->cropTriggerId}").click(function(){
						{$this->getId()}.send('{$this->callbackUrl}','POST',{$customData},{$onServerCropped});
					   });
				});
CROP_HANDLE;
		}
