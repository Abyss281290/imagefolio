...
class AdminPackagesController extends CAdminController
{
	public function accessRules()
	{
		return array(
			...
			array('allow',
				'actions'=>array('index','make','viewModelImages','orderItems','deleteItem','loadContactsList'),
				'roles'=>array('agency','booker'),
			),
			array('allow',
				'roles'=>array('agency'),
				'expression'=>'($package = ModelsPackages::model()->findByPk('.intval(isset($_REQUEST['package_id'])?$_REQUEST['package_id']:$_REQUEST['id']).')) && $package->agency_id == $user->agency_id',
			),
			array('allow',
				'roles'=>array('booker'),
				'expression'=>'($package = ModelsPackages::model()->findByPk('.intval(isset($_REQUEST['package_id'])?$_REQUEST['package_id']:$_REQUEST['id']).')) && $package->booker_id == $user->booker_id',
			),
	...
	public function actionSend($id, $done=0)
	{
		...
		if(isset($_POST['ModelsPackagesSendForm']))
		{
			...
			if($model->validate()) {
				...
				foreach($model->to as $contacts_type => $strings) {
					if($contacts_type == 'unknown') {
						$strings = array_map('trim',explode(',',$strings));
					}
					foreach($strings as $string) {
						if($validator->validateValue($s2 = str_replace(array('<','>'),'',$string))) {
							$contacts[$s2] = array($s2, '');
						} elseif(preg_match('~"?([^"]+)*"?\s<?([^>]+@[^>]+)>?~',$string,$match)) {
							$contacts[$match[2]] = array($match[2],$match[1]);
						}
				...
				foreach($contacts as $contact) {
					...
					// if we have no {package_url} template var, then add package link to the end of body
					if(strpos($body, '{package_url}') === false)
						$body .= CHtml::tag('div',array(),CHtml::link($package_url,$package_url));
					$mailer->Body = strtr($body, array(
					...
					if($mailer->Send()) {
						// save package report
						$report = new ModelsPackagesReports();
						$report->package_id = $package->id;
						$report->agency_id = Yii::app()->user->agency_id;
						$report->sender_role = Yii::app()->user->role;
						$report->sender_user_id = Yii::app()->user->id;
						$report->recipient_name = $contact[1];
						$report->recipient_email = $contact[0];
	...
	public function actionMake()
	{
		if(count($_POST)) {
			...
			{
				$response['errors'][] = 'Select talent or images to make a package';
			}
			else
			{
				$items = array();
	...
	public function actionOrderItems($package_id)
	{
		if(isset($_POST['order'])) {
			$order = 1;
			foreach(explode(',',$_POST['order']) as $item_id) {
				ModelsPackagesItems::model()->updateByPk($item_id, array('order'=>$order));
				$order++;
			}
		}
	}
