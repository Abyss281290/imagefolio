...
class ModelsPackages extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'items' => array(self::HAS_MANY, 'ModelsPackagesItems', 'package_id', 'order'=>'model_id, `order` ASC'),
		);
	}
	...
	public function getOwningAgency()
	{
		return $this->booker_id
				? AgencyBookers::model()->findByPk($this->booker_id)->agency_id
				: $this->agency_id;
	}
	
	public function getAgency()
	{
		return Agencies::model()->findByPk($this->owningAgency);
	}
	
	public function incrementViews()
	{
		$this->views += 1;
		$this->last_viewed_time = new CDbExpression('NOW()');
		$this->update(array('views','last_viewed_time'));
	}
