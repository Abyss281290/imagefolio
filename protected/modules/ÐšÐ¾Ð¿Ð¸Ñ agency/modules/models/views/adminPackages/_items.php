<?php
$this->widget('ext.colorbox.JColorBox')->addInstance('#models-package-images a[rel=model-images]');
?>
<style type="text/css">
.models-tile .items {
	width: auto;
...
<div class="models-tile" id="models-tile">
	<fieldset class="items gallery"><legend>Models</legend>
		<ul>
			<?php
			$first_model_id = null;
			foreach($models as $m):
			...
			?>
				<li class="item<?php if(!$first_model_id) echo ' active'; ?>" id="model-item-<?php echo $m->id; ?>">
					...
					<div class="name"><?php echo CHtml::link($m->fullname, '#', array('onclick'=>$onclick)); ?></div>
					...
					<ul class="img_options">
						<li class="delete"><?php echo CHtml::ajaxLink(
								...
								array(
									...
									'confirm'=>'Delete model from package?',
									'onclick'=>'
