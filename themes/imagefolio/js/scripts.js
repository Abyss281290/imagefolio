...
$.browser.msie&&(console={log:function(str){
	window.status=str
}})
...
$(window).load(function(){
		...
		.tabs({
			...
			,actFu:function(_){				
				if(_.prev)
					...
					,_.curr&&_.curr.stop().css({left:1036})
				else
					_.curr&&_.curr
						.stop()
						.css({left:0})
						
				console.log(_.curr.show)
	...
	win.on('hashchange',checkByCkass)	
	nav
		.on('mouseenter','li',move_nav_hvr)
		.on('mouseleave',checkByCkass)
	function move_nav_hvr(){		
		nav_hvr
			.stop()
			.animate({
				left:0
				,top:this.offsetTop+27
			})				
	}
	
	function checkByCkass(){
		var act=$('.active',nav)
		if(act.length)
			move_nav_hvr.call(act[0])
		else
			nav_hvr
				.stop()
				.animate({
	...
	var _tmp=$('.active',nav)[0]
	if(_tmp)
		move_nav_hvr.call(_tmp)
	...
	function s_subp(){
		...
		bgStretch.bgStretch(templateUrl+'/images/bg-2.jpg')
		frst=false
	}
	...
	function s_splsh(){
		...
		bgStretch.bgStretch(templateUrl+'/images/bg-1.jpg')
		frst=false
	}
...
function include(url){document.write('<script type="text/javascript" src="'+templateUrl+'/'+url+'"></script>')}